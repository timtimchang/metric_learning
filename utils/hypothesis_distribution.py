...
def plot_i_distribution(plt, pos, neg):
...
def plot_h_distribution(plt, h_set):
	x = np.arange(-1, 1.1, 0.1)
	y = np.arange(-1, 1.1, 0.1)
	X, Y = np.meshgrid(x, y)
	Z = np.ones((len(x), len(y)))
...
if __name__ == "__main__":
	...
	sample_hypothesis = [ h - coef for h in np.random.randn(30,3)]
	...
	plot_i_distribution(plt, pos, neg)
	...
	#plot_h_distribution(plt, sel_hypothesis)
	plt.show()
